    ...
    def run(self):
        ...
        while True:
            ...
            if event == sg.WIN_CLOSED or event == 'Cancelar':
                ...
                window.close()
                return(values[0], values[1], values[2], values[3]) # Retorna os três valores como uma tupla.

tela = TelaPython()
login, password, quant, modo = tela.run() # Aqui tu faz o unpack da tua tupla em variáveis.
...
session.unfollow_users(amount=int(quant), instapy_followed_enabled=True, instapy_followed_param="nonfollowers",
                           style="FIFO",
